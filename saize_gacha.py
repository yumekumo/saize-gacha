...
def view():
    ...
    if c_button.button('ガチャを回す'):
        result_ids, total = gacha(price_input, calorie_input)
        c_top.write(menu.loc[result_ids].style.format({'salt':'{:.1f}'}))
        c_top.info("合計 {}円 {}kcal 塩分 {}g".format(total['price'], total['calorie'], round(total['salt'],1)))

    with st.expander('メニューを表示'):
        st.subheader('メニュー表')
        st.write(menu.style.format({'salt':'{:.1f}'}))
        st.markdown('詳しいメニューは[こちら](http://book.saizeriya.co.jp/library/menu1907/book/list)（公式サイト）')
